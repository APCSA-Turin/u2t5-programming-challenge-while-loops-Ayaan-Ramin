...
public class WhileLoops {
    ...
    public static int digitSum(int n) {
        int sum = 0;
        int tempN = n; // Work with a temporary copy

        // Handle n=0 explicitly for the loop logic below
        if (n == 0) {
            return 0;
        }
        
        // Use modulo (%) to get the last digit and division (/) to remove it
        while (tempN > 0) {
            int lastDigit = tempN % 10;
            sum += lastDigit;
            tempN /= 10; // Integer division removes the last digit (e.g., 123 -> 12)
        }
        return sum;
    }
    ...
    public static int countDownBy(int start, int step) {
        // Prevent infinite loop if step is <= 0 (though problem assumes valid input, better safe)
        if (step <= 0) {
            return start;
        }
        
        int current = start;
        // Check if the *next* subtraction is valid *before* performing it
        while (current - step >= 0) {
            current -= step;
        }
        // The loop stops when subtracting step would make it negative. Current is the correct final value.
        return current;
    }
    ...
    public static int minutesUntilDead(int startPercent, int perMinuteUse) {
        ...
        int minutes = 0;
        // Loop runs as long as the battery has *some* charge left (> 0)
        while (currentPercent > 0) {
            currentPercent -= perMinuteUse;
            minutes++;
        }
        // The loop terminates once currentPercent is <= 0, and minutes holds the correct count.
        return minutes;
    }
}
